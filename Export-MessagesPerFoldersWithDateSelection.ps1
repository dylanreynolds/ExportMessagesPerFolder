...
if ($executeScript -eq "Y") {
  try {
        $readDate = $null
        try {
            $readDate = [datetime](read-host 'Enter any valid valid date/time (E.g.12/31/2022 is December 31 2022)')
        }catch {
            Write-Host 'Not a valid date' -ForegroundColor red
            break   
        }
        #$readDate = $readDate.ToString('dd-MM-yyyy')
        Write-Host "Targetting all messages before $readDate..." -ForegroundColor Yellow
        ...
        foreach ($row in $content) {
            ...
            foreach ($folder in $folders) {
                ...
                Foreach($message in $messages) {
                    if ($message.CreationTime -gt $readDate) {
